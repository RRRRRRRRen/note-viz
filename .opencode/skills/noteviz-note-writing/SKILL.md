...
description: 为 NoteViz 学习站创建或修改笔记时使用。强制执行笔记内容规范（一篇一问/知识点拆分/结论先行/类比按需/真实运行输出/可视化密度）与视觉规范（三层内容边界/组件选型/配色语义）。触发词：写笔记、新建笔记、生成笔记、修改笔记、笔记内容、noteviz 笔记。
---
...
产出文件模板：`meta.ts` 用 `satisfies NoteMeta`（五字段：title 为**问句**——疑问主干 + 可选冒号副题，≤25 字 / description / difficulty 入门·进阶·高级 / tags / updated）；`index.tsx` 默认导出 `function Note()`，根元素 `<NoteShell>`，内部是**平铺块流**——`<Heading>` 只放标题本身，段落/图表/列表等块与标题平级依次排列（架构裁决见 BLOCK-SYSTEM.md），私有子组件放同文件底部（超 150 行拆到同目录）。禁止 `<Section>`/`<Prose>` 等结构容器（已 deprecated）。

## 一篇一问：知识点拆分原则（硬性）

笔记按**知识点**组织，不写综合性实操指南。**一篇笔记 = 一个知识点 = 一个问句标题**：

1. **问句标题即学习目标**：学完这篇就能深入回答标题里的问句；结论、机制拆解、追问链、延伸阅读全部围绕回答这一个问句展开。禁止「××精讲」「××大全」「××实战指南」式综合大杂烩。
2. **拆分粒度**：一个核心问句能框住的内容才是一篇——拟标题时若要用「以及 / 和 / 与」连接多个核心问句，就必须拆。同一问句内的机制细节、边界陷阱、追问链属于该问句的深度，不拆。
3. **话题知识点多时先拆分后动笔**：先产出拆分表格（固定表头见下方「大纲确认与详略协商」：标题 | 知识点 | 难度），把每个知识点框成独立一篇，确认后逐篇写。
4. **每个知识点篇的补充与扩展**（两部分标配）：
   - **相邻知识点**：用 `Prerequisite`（前置）与 `CrossRef`（延伸）把知识点篇织成网络——不属于本篇问句的问题，指向对应笔记而不是塞进来
   - **综合性知识**：小型综合应用写成篇末「整合应用」小节（本篇知识点在真实工程中的组合用法）；跨多个知识面的大型串联才独立成整合篇，整合篇标题同样必须是问句
...
3. **大纲提案（固定表格，硬性格式）**：一律用表格与用户确认，固定表头 **标题 | 知识点 | 难度**，每行 = 一篇笔记（一个知识点）；单知识点话题同样用此表格（一行）：
   - **标题**：问句标题，即本篇学习目标
...
- [ ] meta 五字段齐全，title 为问句且 ≤25 字；全篇围绕回答这一个问句
