import { useEffect, useRef, useState } from "react";
import type { ReactNode } from "react";
import { createPortal } from "react-dom";
import { Crosshair, Minimize2, Minus, Plus, Square } from "lucide-react";

const MIN_SCALE = 0.5;
const MAX_SCALE = 3;
const SCALE_STEP = 0.25;

/**
 * 可缩放、可拖拽的画布容器：用于流程图/示意图。
 * 支持内嵌视图与全屏视图（Esc 退出），按钮：放大/缩小/复位/全屏。
 */
export function PanZoomCanvas({ children }: { children: ReactNode }) {
  const [fullscreen, setFullscreen] = useState(false);
  const canvas = (
    <CanvasBody fullscreen={fullscreen} onToggleFullscreen={setFullscreen}>
      {children}
    </CanvasBody>
  );

  if (fullscreen && typeof document !== "undefined") {
    return createPortal(
      <div className="fixed inset-0 z-[100] bg-background">
        <div className="mx-auto flex h-full max-w-[1200px] flex-col px-8 py-6">
          <div className="mb-3 flex items-center justify-between">
            <span className="text-sm font-semibold">示意图 · 全屏查看</span>
            <span className="text-[11px] text-muted meta-mono">Esc 或右下角按钮退出全屏</span>
          </div>
          <div className="min-h-0 flex-1">{canvas}</div>
        </div>
      </div>,
      document.body,
    );
  }
  return canvas;
}

function CanvasBody(props: {
  fullscreen: boolean;
  onToggleFullscreen: (v: boolean) => void;
  children: ReactNode;
}) {
  const { fullscreen, onToggleFullscreen, children } = props;
  const [scale, setScale] = useState(1);
  const [offset, setOffset] = useState({ x: 0, y: 0 });
  const dragRef = useRef<{ startX: number; startY: number; baseX: number; baseY: number } | null>(
    null,
  );
  const [dragging, setDragging] = useState(false);

  // 全屏时监听 Esc 退出
  useEffect(() => {
    if (!fullscreen) return;
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") onToggleFullscreen(false);
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [fullscreen, onToggleFullscreen]);

  const zoom = (dir: 1 | -1) => {
    setScale((s) => Math.min(MAX_SCALE, Math.max(MIN_SCALE, s + dir * SCALE_STEP)));
  };

  const reset = () => {
    setScale(1);
    setOffset({ x: 0, y: 0 });
  };

  const onPointerDown = (e: React.PointerEvent) => {
    (e.target as HTMLElement).setPointerCapture?.(e.pointerId);
    dragRef.current = { startX: e.clientX, startY: e.clientY, baseX: offset.x, baseY: offset.y };
    setDragging(true);
  };
  const onPointerMove = (e: React.PointerEvent) => {
    const d = dragRef.current;
    if (!d) return;
    setOffset({ x: d.baseX + (e.clientX - d.startX), y: d.baseY + (e.clientY - d.startY) });
  };
  const onPointerUp = () => {
    dragRef.current = null;
    setDragging(false);
  };

  const onWheel = (e: React.WheelEvent) => {
    if (!e.ctrlKey && !e.metaKey) return;
    e.preventDefault();
    zoom(e.deltaY < 0 ? 1 : -1);
  };

  return (
    <div
      className={`relative overflow-hidden rounded-lg border border-border bg-card ${
        fullscreen ? "flex h-full flex-col" : "my-4"
      }`}
    >
      <div className="absolute right-2 top-2 z-10 flex gap-1 rounded-md border border-border bg-background/90 p-1 backdrop-blur">
        <ToolButton label="放大" onClick={() => zoom(1)}>
          <Plus size={13} />
        </ToolButton>
        <ToolButton label="缩小" onClick={() => zoom(-1)}>
          <Minus size={13} />
        </ToolButton>
        <ToolButton label="回到原位" onClick={reset}>
          <Crosshair size={13} />
        </ToolButton>
        <ToolButton
          label={fullscreen ? "退出全屏" : "全屏查看"}
          onClick={() => {
            reset();
            onToggleFullscreen(!fullscreen);
          }}
        >
          {fullscreen ? <Minimize2 size={13} /> : <Square size={13} />}
        </ToolButton>
      </div>
      <div
        className={`flex min-h-0 justify-center overflow-auto p-4 ${
          fullscreen ? "flex-1 items-center" : "min-h-48"
        } ${dragging ? "cursor-grabbing" : "cursor-grab"}`}
        onPointerDown={onPointerDown}
        onPointerMove={onPointerMove}
        onPointerUp={onPointerUp}
        onPointerLeave={onPointerUp}
        onWheel={onWheel}
      >
        <div
          style={{
            transform: `translate(${offset.x}px, ${offset.y}px) scale(${scale})`,
            transformOrigin: "center top",
          }}
          className="origin-top"
        >
          {children}
        </div>
      </div>
      <div className="border-t border-border px-3 py-1.5 text-[10px] text-muted meta-mono">
        拖拽移动 · Ctrl/⌘ + 滚轮缩放 · {Math.round(scale * 100)}%{fullscreen ? " · Esc 退出" : ""}
      </div>
    </div>
  );
}

function ToolButton({
  label,
  onClick,
  children,
}: {
  label: string;
  onClick: () => void;
  children: ReactNode;
}) {
  return (
    <button
      type="button"
      onClick={onClick}
      title={label}
      aria-label={label}
      className="rounded p-1.5 text-muted hover:bg-surface-2 hover:text-foreground"
    >
      {children}
    </button>
  );
}
