  ...
  return (
    <NoteShell>
      ...
      <Section title="一个生活化的类比">
        <Prose>
          <p>
            执行上下文栈就是<strong>餐厅的传菜口</strong>，永远只处理最上面那一盘：
          </p>
          <p>
            开业时放上一个永远在的底盘（全局上下文）；每点一道菜（调用函数），服务员把新盘子
            <strong>压</strong>上传菜口；这道菜做完（函数返回）盘子端走，露出下面的盘子继续出餐。
            盘子叠得越高（递归越深），传菜口压力越大——叠爆了就是栈溢出（stack overflow）。
          </p>
        </Prose>
      </Section>

      <Section title="执行上下文的类型">
        <Prose>
          <p>
...
// Inside global execution context`}
        ...
        <Prose>
          <p>
            ...
            <code>[global, first, second]</code> → <code>[global, first]</code> →{" "}
            <code>[global]</code>。注意 first 的后半段要等 second 弹栈后才能继续——这就是"栈顶优先"。
          </p>
        </Prose>
      </Section>

      <Section title="上下文的三个阶段">
        <Subsection title="创建阶段">
          <Prose>
            <p>
        ...
        <Subsection title="执行阶段">
          <Prose>
            <p>逐行执行代码，完成变量赋值与函数调用。</p>
          </Prose>
        </Subsection>
        <Subsection title="回收阶段">
          <Prose>
            <p>
...
var bar = function () {
  console.log('I am a function expression');
        ...
        <Prose>
          <p>
            <strong>运行结果解读</strong>：<code>var bar</code> 提升后是 <code>undefined</code>，
            调用 <code>undefined()</code> 自然抛 TypeError——注意这与 let 的 TDZ
